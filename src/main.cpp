...
/** Fees smaller than this (in uafo) are considered zero fee (for relaying and mining)
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        if (isBlockFromFork) {
            ...
            if(hasZAFOInputs){
                for (const CTxIn& zPivInput : zAFOInputs) {
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zAFO mints and %d zAFO spends\n", __func__, nMints, nSpends);
