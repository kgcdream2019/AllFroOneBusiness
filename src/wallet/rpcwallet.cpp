...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    for (const string& name_ : keys) {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid AllForOneBusiness address: ")+name_);
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zafoTracker->ListMints(true, fMatureOnly, true);
...
UniValue listzerocoinamounts(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zafoTracker->ListMints(true, true, true);
...
extern UniValue DoZafoSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool ispublicspend)
{
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzAFOTracker* zafoTracker = pwalletMain->zafoTracker.get();
    set<CMintMeta> setMints = zafoTracker->ListMints(false, false, false);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CzAFOTracker* zafoTracker = pwalletMain->zafoTracker.get();
    set<CMintMeta> setMints = zafoTracker->ListMints(!fIncludeSpent, false, false);
